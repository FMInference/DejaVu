...
MODEL_CONF="--seq-length 2048 --embedding-dim 5120 --num-heads 40 --num-layers $num_layers --batch-size $batch_size --micro-batch-size 1"

python3 dist_training_runner.py --dist-url tcp://"$ip":9000 --fp16 $DIST_CONF $MODEL_CONF >> "./logs/${timestamp}_GPT3_13B_world32_rank${rank}_L${num_layers}_B${global_batch_size}_fluidStack_${mode}.log"
